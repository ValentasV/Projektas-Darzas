...
def search(request):
    if request.method == "POST":
        ...
        augalas = Augalas.objects.filter(Q(pavadinimas__icontains=searched) | Q(veisle__icontains=searched)).filter(naudotojas=request.user)
        darzodarbas = DarzoPrieziura.objects.filter(darzodarbai__pavadinimas__icontains=searched, darzodarbai__naudotojas=request.user)
...
class AugaloRedagavimasView(LoginRequiredMixin, UpdateView):
    form_class = AugaloForm
    template_name = "augalas/augalo_redagavimas.html"
    success_url = "/augalai/"
    def form_valid(self, form):
        form.save()
        return super().form_valid(form)
